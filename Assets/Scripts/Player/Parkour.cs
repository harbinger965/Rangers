...
namespace Assets.Scripts.Player
{
	public class Parkour : ControllerObject 
	{
		...
		public void Locomote(float motion) 
		{
			...
			{
   				if(facingRight && Physics.Raycast(new Ray(transform.position + new Vector3(0.2f, 0, 0), Vector3.up),2.5f,~(1<<9))) 
				{
					animator.SetFloat("RunSpeed", Mathf.Min(0,motion));
				} 
				else if(!facingRight && Physics.Raycast(new Ray(transform.position - new Vector3(0.2f, 0, 0), Vector3.up), 2.5f,~(1<<9))) 
				{
